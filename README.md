...
Usage is very straight forward, simply call `verify_pass_barcode` with the scanned QR Code URI. If the given pass is valid the function will return `Ok` with the pass details, otherwise it will return `Err` with details of the verification issue.
...
let pass: PublicCovidPass = verify_pass_barcode(barcode).await?;
```
